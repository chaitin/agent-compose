...
Project RPC Git sources may use remote repositories (including internal hosts and the existing SSH/Git transports) or daemon-local repositories. Local paths and `file://` URLs must select a repository inside the project source directory after symlink resolution. Relative repository paths are resolved against that directory, not the daemon working directory. Supply an absolute `source.compose_path` or `source.project_dir`; the directory must exist on the daemon. A compose file path selects its parent directory, and `source.compose_path` takes precedence when both fields are present. PatchProject uses the saved project source path. Missing source directories and repositories outside that directory are rejected before Git retrieval. The script's `path` must also stay inside the checked-out repository. These checks define the resource location contract; the daemon API remains an administrative trust boundary, not an isolation boundary between untrusted callers.

ValidateProject, ApplyProject, and PatchProject resolve sources on the daemon, including dry runs. The existing bounded resolver propagates request cancellation and retains its timeout, size, redirect, and UTF-8 checks. Fetch/validation failures do not save a revision. Normalization trims leading/trailing whitespace and removes a leading BOM from fetched source text; it does not interpolate environment references in script text. Successful application persists the normalized inline script snapshot and returns it in `script`; source credentials and source metadata are not retained. Subsequent scheduler execution and GetProject do not fetch the source again. A later request containing `script_source` fetches it again.
