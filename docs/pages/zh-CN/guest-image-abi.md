...
官方默认 guest 是一个覆盖面较广的运行环境，但有意不包含 Go 工具链，也不包含 `grpcurl`、`protoc-gen-go` 和 `protoc-gen-go-grpc` 等 Go 构建工具。Baseline ABI 不要求特定 Linux 发行版、`apt`、Go、C/C++ compiler、这些 Go 工具、Git、curl、`tini`、所有 provider CLI、Jupyter、额外 kernel 或离线 runtime SDK。
