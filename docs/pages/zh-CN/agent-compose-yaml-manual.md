...
- 只接受 `http` 与 `https` URL，重定向同样按该规则重新校验，并且该请求会忽略代理环境变量，避免代理代替 daemon 解析目标地址。支持内网制品服务器：URL 可以解析到 loopback 或私网地址，例如另一个 Docker Compose 服务名、发布到本机环回的端口，或 daemon 私网内的主机。（Skill 解析不允许私网地址；Workspace 压缩包 URL 由部署该项目的运维人员编写。）
