...
type scheduledRunDispatcher struct {
	deps scheduledRunDispatcherDeps
}

func newScheduledRunDispatcher(deps scheduledRunDispatcherDeps) *scheduledRunDispatcher {
	if deps.RootCtx == nil {
		deps.RootCtx = context.Background()
	}
	return &scheduledRunDispatcher{deps: deps}
}

func (s *scheduledRunDispatcher) Loop() {
	for {
...
func (s *scheduledRunDispatcher) Dispatch(jobs []ScheduledRun) {
...
func (s *scheduledRunDispatcher) NextFireAt() (time.Time, bool) {
...
func (s *scheduledRunDispatcher) CollectDue(now time.Time) []ScheduledRun {
...
func (s *scheduledRunDispatcher) snapshot() map[string]domain.Loader {
...
func (s *scheduledRunDispatcher) rootCtx() context.Context {
...
func (s *scheduledRunDispatcher) runTimeout(override time.Duration) time.Duration {
