...
func newTestLLMFacadeService(t *testing.T) (*Service, *Store, *ConfigStore) {
	t.Helper()
	root := t.TempDir()
	config := &appconfig.Config{
		DataRoot:           root,
		SessionRoot:        filepath.Join(root, "sessions"),
		DefaultImage:       "llm-facade:latest",
	...
	configDB, err := storage.NewConfigStoreFromConfig(&appconfig.Config{
		DataRoot: root,
	...
	client := NewClient(config, configDB, llmServer.Client())
	return NewService(config, store, configDB, client), store, configDB
}
...
func TestRuntimeLLMFacadeForwardsWithSessionToken(t *testing.T) {
	...
	service, _, _ := newTestLLMFacadeService(t)
	...
	service.client.client = upstream.Client()
...
func TestFilterPersistedRuntimeEnvFiltersLLMProviderKeys(t *testing.T) {
	filtered := filterPersistedRuntimeEnv([]SessionEnvVar{
...
func TestIntegrationRuntimeLLMFacadeRoundTrips(t *testing.T) {
