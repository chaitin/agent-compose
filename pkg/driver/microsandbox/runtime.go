...
import (
	...
	dockerprobe "agent-compose/pkg/driver/internal/dockerprobe"
	envpath "agent-compose/pkg/driver/internal/envpath"
	execfilter "agent-compose/pkg/driver/internal/execfilter"
...
type microsandboxExecCollector struct {
	...
	filter *execfilter.ExecOutputFilter
	stdout bytes.Buffer
	stderr bytes.Buffer
	output bytes.Buffer
}

func NewRuntime(config *appconfig.Config) (BoxRuntime, error) {
...
func (r *microsandboxRuntime) EnsureSession(ctx context.Context, session *Session, vmState VMState, proxyState ProxyState) (SessionVMInfo, error) {
	...
	if !needLaunch {
		...
		probeErr := boxlitedriver.WaitForJupyterProxy(probeCtx, proxyState)
		cancel()
	...
	readyErr := boxlitedriver.WaitForJupyterProxy(readyCtx, proxyState)
	cancel()
	if readyErr != nil {
		if logText := boxlitedriver.ReadSessionJupyterLog(session); boxlitedriver.JupyterLogIndicatesReady(logText) {
	...
	return SessionVMInfo{
		BoxID:      name,
		JupyterURL: boxlitedriver.JupyterDirectURL(proxyState),
	}, nil
}
...
func (r *microsandboxRuntime) ExecStream(ctx context.Context, session *Session, vmState VMState, spec ExecSpec, stream ExecStreamWriter) (ExecResult, error) {
	...
	collector := &microsandboxExecCollector{stream: stream, filter: execfilter.New()}
	exitCode := 0
	sawExit := false
	for {
...
func (r *microsandboxRuntime) prepareEnvironment() error {
	...
	envpath.Prepend("PATH", filepath.Dir(r.config.MicrosandboxMSBPath))
	envpath.Prepend("LD_LIBRARY_PATH", filepath.Dir(r.config.MicrosandboxLibPath))
...
func (r *microsandboxRuntime) createSandbox(ctx context.Context, session *Session, vmState VMState, proxyState ProxyState, name string) (*microsandbox.Sandbox, error) {
	...
	manifest, err := drivermount.LoadDirectoryRuntimeMountManifest(session, RuntimeDriverMicrosandbox)
	if err != nil {
		return nil, err
	}
	...
	imageRef := driverimage.ResolveSessionGuestImage(vmState.Image, session.Summary.GuestImage, DefaultGuestImageForDriver(r.config, RuntimeDriverMicrosandbox))
...
func (r *microsandboxRuntime) resolveMicrosandboxImageRef(ctx context.Context, imageRef string) (string, bool, error) {
	rootfs, ok, err := resolveMicrosandboxRootFS(ctx, imageRef, microsandboxImageResolverOps{
		dockerAvailable: dockerprobe.Available,
		dockerMaterialize: func(ctx context.Context, imageRef string) (microsandboxRootFSResult, bool, error) {
			rootfs, ok, err := driverimage.MaterializeLocalDockerImageRootfs(ctx, r.config.DataRoot, imageRef)
			if err != nil || !ok {
...
func (r *microsandboxRuntime) launchJupyter(ctx context.Context, sandbox *microsandbox.Sandbox, proxyState ProxyState) error {
	command := boxlitedriver.JupyterLaunchCommand(r.config, proxyState, true)
