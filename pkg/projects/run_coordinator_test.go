...
func setupRunPreparationProject(t *testing.T, spec *agentcomposev2.ProjectSpec, projectDir string) (*ConfigStore, *Service, string) {
	...
	streams := newProjectTestStreamBroker(t)
	service.streams = streams
	service.executor = newProjectTestExecutor(service.config, service.store, store, runtimes, streams)
	registerProjectTestRuntime(service, runtime)
	ctx := context.Background()
