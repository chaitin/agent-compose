...
// ProjectSpecToProtoWithRedactedOctoBusTokens preserves the existing project
// response shape while hiding the inherently secret OctoBus credentials.
func ProjectSpecToProtoWithRedactedOctoBusTokens(spec *compose.NormalizedProjectSpec) *agentcomposev2.ProjectSpec {
	if spec == nil {
		return nil
	}
	return RedactProjectOctoBusTokens(ProjectSpecToProto(spec))
}

// RedactProjectOctoBusTokens returns a copy with project OctoBus tokens hidden.
func RedactProjectOctoBusTokens(spec *agentcomposev2.ProjectSpec) *agentcomposev2.ProjectSpec {
	if spec == nil {
		return nil
	}
