...
func TestSpecHashIncludesAgentLLMConnection(t *testing.T) {
	withoutConnection := mustNormalizeCompose(t, `
name: llm-connection-hash
agents:
  reviewer:
    provider: codex
    model: gpt-5.5
`, nil)
	withConnection := mustNormalizeCompose(t, `
name: llm-connection-hash
agents:
  reviewer:
    provider: codex
    model: gpt-5.5
    llm_connection: gateway
`, nil)
	otherConnection := mustNormalizeCompose(t, `
name: llm-connection-hash
agents:
  reviewer:
    provider: codex
    model: gpt-5.5
    llm_connection: backup
`, nil)

	if got := withoutConnection.Agents[0].LLMConnection; got != "" {
		t.Fatalf("normalized llm_connection = %q, want empty when it is not declared", got)
	}
	if got := withConnection.Agents[0].LLMConnection; got != "gateway" {
		t.Fatalf("normalized llm_connection = %q, want gateway", got)
	}
	// Canonical JSON is the immutable representation a revision is read back
	// from, so the field must survive a marshal/parse round trip.
	data, err := withConnection.MarshalCanonicalJSON(false)
	if err != nil {
		t.Fatalf("MarshalCanonicalJSON returned error: %v", err)
	}
	if !strings.Contains(string(data), `"llm_connection":"gateway"`) {
		t.Fatalf("canonical JSON %s does not carry llm_connection", data)
	}
	parsed, err := ParseCanonicalJSON(data)
	if err != nil {
		t.Fatalf("ParseCanonicalJSON returned error: %v", err)
	}
	if got := parsed.Agents[0].LLMConnection; got != "gateway" {
		t.Fatalf("round-tripped llm_connection = %q, want gateway", got)
	}
	if got := withConnection.Redacted().Agents[0].LLMConnection; got != "gateway" {
		t.Fatalf("redacted llm_connection = %q, want gateway", got)
	}
	if mustHash(t, withConnection) == mustHash(t, withoutConnection) {
		t.Fatalf("hash did not change when llm_connection was declared")
	}
	if mustHash(t, otherConnection) == mustHash(t, withConnection) {
		t.Fatalf("hash did not change when llm_connection changed")
	}
}

func TestNormalizeInterpolatesAgentLLMConnection(t *testing.T) {
	normalized := mustNormalizeCompose(t, `
name: llm-connection-interpolation
agents:
  reviewer:
    provider: codex
    model: gpt-5.5
    llm_connection: ${REVIEWER_CONNECTION}
`, map[string]string{"REVIEWER_CONNECTION": "gateway"})

	if got := normalized.Agents[0].LLMConnection; got != "gateway" {
		t.Fatalf("interpolated llm_connection = %q, want gateway", got)
	}
}
