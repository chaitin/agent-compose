...
func (c *Controller) prepareProjectRunWorkspace(ctx context.Context, run domain.ProjectRunRecord, project domain.ProjectRecord, req WorkspaceRequest) (*domain.WorkspaceConfig, error) {
	...
	case sources.ProviderFile:
		config, err := c.materializeLocalProjectRunWorkspace(ctx, run, project, workspace)
		if err != nil {
			return nil, err
		}
		return &config, nil
...
func (c *Controller) materializeLocalProjectRunWorkspace(ctx context.Context, run domain.ProjectRunRecord, project domain.ProjectRecord, workspace *compose.WorkspaceSpec) (result domain.WorkspaceConfig, retErr error) {
	...
	config, err = workspaces.CreateTransientFileSnapshot(ctx, c.config, config)
	if err != nil {
		return domain.WorkspaceConfig{}, err
	}
	defer func() {
		if retErr != nil {
			retErr = errors.Join(retErr, workspaces.ReleaseTransientSnapshot(context.WithoutCancel(ctx), c.config, toSandboxWorkspaceSnapshot(config)))
		}
	}()
