...
func TestStickyProjectRunHashTracksEveryWorkspaceMountField(t *testing.T) {
	...
	hash := func(item workspaces.FileWorkspaceMountConfig) string {
		...
		value, err := stickyProjectRunConfigHash("scheduler", domain.ProjectRunRecord{ProjectID: "project"}, Preparation{
			Workspace: &domain.SandboxWorkspace{ID: "workspace", Type: "file", ConfigJSON: string(payload)},
		}, stickySandboxSpec{Driver: "docker"})
		if err != nil {
			t.Fatal(err)
		}
		return value
