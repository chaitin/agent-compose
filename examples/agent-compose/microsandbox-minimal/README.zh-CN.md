...
本示例定义一个使用 Microsandbox driver 的 Codex agent。
...
归一化输出应包含 `driver.name: microsandbox`。

## 在 Microsandbox host 上运行
...
未包含 Microsandbox，命令会报告 driver 不受支持；缺少 KVM 权限或 runtime
artifacts 会导致 Microsandbox 初始化失败。
...
该输出表明 project 已选择 Microsandbox；runtime 输出取决于 host 上的
Microsandbox artifacts 和 KVM 环境。
