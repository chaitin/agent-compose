...
const facadeTokenRetention = time.Hour
...
func (s *Store) EnsureSchema(ctx context.Context) error {
	...
	if err := configdomain.EnsureColumn(ctx, s.db, "llm_provider", "use_generic_responses_text_parts", "INTEGER NOT NULL DEFAULT 0"); err != nil {
...
func (s *Store) UpsertDefaultLLMConfig(ctx context.Context, provider Provider, model Model) error {
	...
	if _, err := tx.ExecContext(ctx, `INSERT INTO llm_model(id, name, description, default_model, enabled, scope, created_at, updated_at)
		VALUES(?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET name = excluded.name, description = excluded.description, default_model = excluded.default_model, enabled = excluded.enabled, scope = excluded.scope, updated_at = excluded.updated_at`, model.ID, model.Name, model.Description, configdomain.BoolToInt(model.DefaultModel), configdomain.BoolToInt(model.Enabled), model.Scope, now, now); err != nil {
...
func (s *Store) ListEnabledLLMProviders(ctx context.Context) ([]Provider, error) {
	...
	for rows.Next() {
		item, err := scanProvider(rows.Scan)
		if err != nil {
			return nil, err
		}
...
func (s *Store) ListEnabledLLMModels(ctx context.Context) ([]Model, error) {
...
func (s *Store) LLMProviderModelWireAPI(ctx context.Context, providerID, modelID string) (string, bool, error) {
	...
	return NormalizeWireAPI(wireAPI), true, nil
}

func (s *Store) SaveLLMFacadeToken(ctx context.Context, token FacadeToken) error {
...
func (s *Store) DeleteLLMFacadeToken(ctx context.Context, rawToken string) error {
	...
	hash, _ := HashFacadeToken(rawToken)
...
func (s *Store) GetLLMFacadeToken(ctx context.Context, rawToken string) (FacadeToken, error) {
	hash, fingerprint := HashFacadeToken(rawToken)
	...
	token, err := scanFacadeToken(row.Scan)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return FacadeToken{}, fmt.Errorf("llm facade token %s not found: %w", fingerprint, err)
		}
...
func (s *Store) RevokeLLMFacadeTokensForSession(ctx context.Context, sessionID string) error {
	...
	cutoff := now.Add(-facadeTokenRetention).Unix()
...
func scanProvider(scan func(dest ...any) error) (Provider, error) {
	...
	item.CreatedAt = configdomain.ParseStoredTime(createdAt)
	item.UpdatedAt = configdomain.ParseStoredTime(updatedAt)
	return item, nil
}
...
func scanFacadeToken(scan func(dest ...any) error) (FacadeToken, error) {
	...
	item.IssuedAt = configdomain.ParseStoredTime(issuedAt)
	item.ExpiresAt = configdomain.ParseStoredTime(expiresAt)
	item.RevokedAt = configdomain.ParseStoredTime(revokedAt)
	return item, nil
}
