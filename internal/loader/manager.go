...
type LoaderRunHostForTest = loaderRunHost

func NewLoaderRunHostForTest(manager *LoaderManager, loader Loader, run *LoaderRunSummary) *LoaderRunHostForTest {
	return &loaderRunHost{manager: manager, loader: loader, run: run}
}
...
func NewLoaderManager(di do.Injector) (*LoaderManager, error) {
	...
	dashboard, _ := do.Invoke[dashboardNotifier](di)
	m := &LoaderManager{
		config:       config,
		rootCtx:      rootCtx,
...
func (m *LoaderManager) initLoaderComponents() {
	...
	if m.projectAgentRunner == nil {
		m.projectAgentRunner = unavailableProjectAgentRunner{}
	}
}

func NewLoaderManagerForTest(config *appconfig.Config, configDB ConfigStore, engine LoaderEngine) *LoaderManager {
...
func (m *LoaderManager) SetStoreForTest(store *Store) {
	m.store = store
}

func (m *LoaderManager) SetLLMClientForTest(client *LLMClient) {
	m.llm = client
}

func (m *LoaderManager) SetBusForTest(bus *LoaderBus) {
	m.bus = bus
}

func (m *LoaderManager) SetEngineForTest(engine LoaderEngine) {
	m.engine = engine
}

func (m *LoaderManager) SetDriverForTest(driver Driver) {
	m.driver = driver
}

func (m *LoaderManager) SetExecutorForTest(executor *Executor) {
	m.executor = executor
}

func (m *LoaderManager) SetSessionStreamBrokerForTest(streams *SessionStreamBroker) {
	m.streams = streams
}
...
func (m *LoaderManager) StartEventLoopForTest() {
	go m.eventLoop()
}
...
func (h *loaderRunHost) Agent(ctx context.Context, prompt string, request LoaderAgentRequest) (LoaderAgentResult, error) {
	...
	if strings.TrimSpace(request.Agent) != "" {
		agentConfig.Provider = normalizeAgentKind(request.Agent)
	}
...
func (m *LoaderManager) HasEngine() bool {
	return m != nil && m.engine != nil
}
