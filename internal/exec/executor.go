...
const DefaultLoaderCommandMaxOutputBytes = defaultLoaderCommandMaxOutputBytes
...
func NewExecutorForTest(config *appconfig.Config, store Store, configDB ConfigStore, runtimes RuntimeProvider, streams SessionStreamBroker) *Executor {
	if streams == nil {
		streams = noopSessionStreamBroker{}
	}
	return &Executor{config: config, store: store, configDB: configDB, runtimes: runtimes, streams: streams}
}

type noopSessionStreamBroker struct{}

func (noopSessionStreamBroker) PublishCellStarted(string, NotebookCell)        {}
func (noopSessionStreamBroker) PublishCellOutput(string, string, string, bool) {}
func (noopSessionStreamBroker) PublishCellCompleted(string, NotebookCell)      {}
func (noopSessionStreamBroker) PublishEventAdded(string, SessionEvent)         {}
...
type ExecStreamAccumulator = execStreamAccumulator
...
func GuestSessionHome(config *appconfig.Config) string {
	return guestSessionHome(config)
}
...
func MergeExecResults(primary, fallback ExecResult) ExecResult {
	return mergeExecResults(primary, fallback)
}
